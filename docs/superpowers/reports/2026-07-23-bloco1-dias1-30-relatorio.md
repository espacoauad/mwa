...
| 30 | lanchesProteicos.js | Entrada `{ dia: 30, ... 'Prato-celebração proteico' ... }` (linhas 232-242) | Este arquivo cobre oficialmente os dias 1-21 (conforme o próprio comentário do topo do arquivo: "Dias 22-30... ver lanches22a37.js"), mas contém uma entrada extra para o dia 30 com um lanche diferente ("Prato-celebração proteico") do que já existe em `lanches22a37.js` para o mesmo dia ("Whey com banana e aveia"). Verificado no roteamento real (`src/components/dicas/Dicas.jsx` linha 31): o app principal usa `lancheDoDia22a37` para o dia 30, então essa entrada duplicada em `lanchesProteicos.js` é órfã/morta ali — MAS a rota de preview interna (`src/components/dicas/PreviewLanches.jsx`, `/preview-lanches`) usa apenas `lancheDoDia` de `lanchesProteicos.js` e por isso mostraria esse conteúdo diferente do que aparece no app para o dia 30. Além disso, essa entrada não tem quantidades nos `ingredientes` ("Cubos de queijo", "Peito de peru ou frango" sem gramatura), então o valor `proteina: '~28g'` não é verificável por soma. Não removido/editado por ser uma decisão estrutural (qual conteúdo é o "oficial" do dia 30, e se a entrada duplicada deve ser removida) fora do escopo de edição de texto/fatos desta task — sinalizado para decisão da Wanessa |

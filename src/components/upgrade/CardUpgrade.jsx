...
import { faseUpgrade, PRODUTOS, formatarPreco, linkSessao } from '../../utils/ofertas.js'
...
// disponível do dia 6 ao dia 30 (mesmo preço de R$ 97), e novamente como
// "nova oportunidade" a partir do dia 30. Não existe produto "90 dias" à
// parte — é um upgrade que libera os dias 31–90 do mesmo programa, vendido
// sempre via checkout Hotmart (abrirCheckout('programa90d')).
    ...
    return (
      <section className="mt-4 overflow-hidden rounded-2xl border-2 border-ouro/50 bg-ouro-claro p-5 shadow-sm shadow-verde/5">
        ...
        <h2 className="mt-3 font-serif text-xl font-semibold italic text-verde">
          MWA | Upgrade para 90 Dias
        </h2>
        <p className="mt-1 text-sm text-verde/70">
          {ingles ? 'Unlock days 31–90 with extended support — 90 days of continuous transformation.' : 'Libere os dias 31 a 90 com acompanhamento estendido — 90 dias de transformação contínua.'}
        </p>
        <div className="mt-3 flex items-baseline gap-2">
          <span className="text-3xl font-bold text-verde">{formatarPreco(PRODUTOS.upgrade.preco)}</span>
        </div>
        ...
        <button
          ...
          className="mt-4 block w-full rounded-lg bg-ouro px-6 py-4 text-center font-semibold text-verde-escuro transition-all hover:brightness-95 active:scale-[0.98]"
        >
          {ingles ? 'Unlock 90 days' : 'Garantir upgrade de 90 dias'}
        </button>
      </section>
    )
  }
    ...
    return (
      <section className="mt-4 rounded-2xl bg-white p-5 shadow-sm shadow-verde/5">
        <h2 className="flex items-center gap-2 font-semibold text-verde">
          <CalendarHeart size={18} className="text-sage" /> MWA | Upgrade para 90 Dias
        </h2>
        <p className="mt-1 text-sm text-verde/60">
          {ingles ? 'Unlock days 31–90 with continued support' : 'Libere os dias 31 a 90 com acompanhamento contínuo'} — {formatarPreco(PRODUTOS.upgrade.preco)}.
        </p>
        <button
          type="button"
          onClick={() => abrirCheckout('programa90d')}
          className="mt-3 block w-full rounded-lg border-2 border-sage/40 bg-white px-6 py-3.5 text-center text-sm font-semibold text-verde transition-colors hover:border-sage"
        >
          {ingles ? 'Unlock for' : 'Garantir por'} {formatarPreco(PRODUTOS.upgrade.preco)}
        </button>
      </section>
    )
  }

  // fase final — dia 30+: nova oportunidade, mesmo preço de upgrade
  return (
      ...
      <div className="mt-4 flex flex-col gap-2.5">
        <div>
          <button
            type="button"
            onClick={() => abrirCheckout('programa90d')}
            className="block w-full rounded-lg bg-ouro p-4 text-left text-verde-escuro transition-all hover:brightness-95 active:scale-[0.99]"
          >
            ...
            <span className="mt-0.5 block font-serif text-lg font-semibold italic">
              MWA | Upgrade para 90 Dias — {formatarPreco(PRODUTOS.upgrade.preco)}
            </span>
          </button>
        </div>

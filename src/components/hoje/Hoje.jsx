...
import { Plus, GlassWater, Flame, Lightbulb, ChevronRight, Newspaper, Share2, PartyPopper } from 'lucide-react'
...
export default function Hoje({ irParaDicas }) {
  ...
  async function compartilhar() {
    const texto = `Estou no dia ${diaAtual} da minha transformação com o MWA — Método Wanessa Auad! 🌿 Vem comigo: https://metodomwa.com.br`
    try {
      if (navigator.share) {
        await navigator.share({ title: 'MWA — Método Wanessa Auad', text: texto })
      } else {
        await navigator.clipboard.writeText(texto)
        setAvisoCompartilhar('Texto copiado! Cole nos seus stories do Instagram 📲')
  ...
  const progresso = Math.round((diaAtual / 21) * 100)
  const dica = dicaDoDia(diaAtual)
  ...
  const linhasResumo = [
    { label: 'Calorias', consumido: totaisHoje.calorias, meta: metas.calorias, unidade: 'kcal' },
    { label: 'Proteína', consumido: totaisHoje.proteina, meta: metas.proteina, unidade: 'g' },
    { label: 'Carboidratos', consumido: totaisHoje.carbos, meta: metas.carboidrato, unidade: 'g' },
    { label: 'Gordura', consumido: totaisHoje.gordura, meta: metas.gordura, unidade: 'g' },
    { label: 'Fibras', consumido: totaisHoje.fibras, meta: metas.fibras, unidade: 'g' },
  ]
    ...
    <div>
      ...
      <header className="bg-verde px-6 pb-14 pt-10 text-white">
        ...
        <div className="mt-2 flex items-start justify-between gap-3">
          <div>
            ...
            <h1 className="mt-1 font-serif text-2xl font-semibold italic">Olá, {primeiroNome} 🌿</h1>
          </div>
          {/* Avatar + sementes: toque abre a loja */}
          {game && (
            <button
              type="button"
        ...
        <div className="mt-5 rounded-2xl bg-white/10 p-4">
          <div className="flex items-baseline justify-between">
            <p className="font-semibold">
              Dia <span className="text-2xl text-ouro">{diaAtual}</span> de 21
            </p>
            <p className="text-sm text-white/70">{progresso}% do programa</p>
          </div>
      ...
      <main className="-mt-8 px-5">
        ...
        {diaCompleto && (
          ...
          >
            ...
            <span className="flex-1">
              <span className="block text-sm font-bold">Dia {diaAtual} concluído! 🎉</span>
              <span className="text-xs text-white/80">Toque para ver e compartilhar sua conquista</span>
            </span>
        ...
        <section className="rounded-2xl bg-white p-6 shadow-lg shadow-verde/10">
          <h2 className="mb-1 text-sm font-semibold text-verde/60">Seu consumo de hoje</h2>
          <p className="mb-6 text-xs text-verde/40">Macronutrientes em destaque — proteína é o foco</p>
          <GraficoMacros
          ...
          <div className="mt-6 border-t border-cinza pt-4">
            <AnelMeta label="Fibras" consumido={totaisHoje.fibras} meta={metas.fibras} unidade="g" />
          </div>
        </section>
        ...
        <section className="mt-4 rounded-2xl bg-white p-5 shadow-sm shadow-verde/5">
          <h2 className="text-sm font-semibold text-verde/60">Resumo do dia</h2>
            ...
            {linhasResumo.map((l) => (
              <li key={l.label} className="flex items-center justify-between py-2.5 text-sm">
                ...
                <span className="text-verde/60">
                  <strong className="text-verde">{l.consumido.toLocaleString('pt-BR')}</strong> /{' '}
                  {l.meta.toLocaleString('pt-BR')} {l.unidade}
                </span>
              </li>
            ))}
            <li className="flex items-center justify-between py-2.5 text-sm">
              <span className="flex items-center gap-1.5 font-medium text-verde/80">
                <Flame size={15} className="text-ouro" /> Gasto com exercícios
              </span>
              <strong className="text-verde">{gastoExercicios.toLocaleString('pt-BR')} kcal</strong>
            </li>
          </ul>
        </section>
        ...
        >
          ...
          <span className="flex-1">
            <span className="flex items-center gap-1 text-xs font-semibold text-white/60">
              <Newspaper size={12} /> Informativo do dia {diaAtual}
            </span>
        ...
        >
          ...
          <span className="flex-1">
            <span className="flex items-center gap-1 text-xs font-semibold text-verde/60">
              <Lightbulb size={12} /> Dica do dia {diaAtual}
            </span>
        ...
        >
          ...
          <span className="flex-1">
            ...
            <span className="text-xs text-verde/60">
              Poste nos stories e ganhe <strong className="text-verde">+15 🌱</strong> por dia
            </span>
          </span>
          <ChevronRight size={18} className="text-verde/40" />
        </button>
        ...
        {/* Oferta de upgrade conforme o cronograma */}
        <CardUpgrade dia={diaAtual} />
      </main>
